...
def score_pairs(samples_pairwise):
...
# set paths
script_path = './'
sim_path = '/home/kraken/Dropbox/Ryan_Grad/collabs/unrelated_sim_out/sims/'

# make list of sim files
list_dir = os.listdir(sim_path)
#sim_list = [x for x in list_dir if x.find('.csv') != -1 
#	and x.find('_score') == -1
#	]
...
for file in sim_list:
	...
	# build scoring matrix from pairwise list
	score_data = score_pairs(samples_pairwise)
	...
	#hist_df = pd.DataFrame(score_df.corrected_score).convert_objects(convert_numeric=True)
	hist_df = score_df.corrected_score.convert_objects(convert_numeric=True)
	# Normalize the histogram data
	norm_hist_df = np.round((hist_df - hist_df.mean())/(hist_df.max() - hist_df.min()),decimals=2)
	score_df['normalized_score'] = norm_hist_df
	...
	# out the scoring dataframe for safe keeping
	score_df.to_csv(file.split('.')[0] + '_score.csv')
